...
export default function AdvancedCanvas() {
  ...
  const renderComponentContent = component => {
    ...
    switch (component.type) {
      ...
      case 'checkbox':
        return (
          <div className="w-full h-full flex items-center gap-2">
            <div
              className={`w-4 h-4 border-2 rounded flex items-center justify-center ${props.checked ? 'bg-primary border-primary' : 'border-gray-300'}`}
            >
      ...
      case 'toggle':
        return (
          <div className="w-full h-full flex items-center gap-3">
            <div
              className={`w-10 h-6 rounded-full relative cursor-pointer transition-colors ${props.checked ? 'bg-primary' : 'bg-gray-300'}`}
            >
              <div
              ...
              {(props.options || ['Option 1', 'Option 2'])
                ...
                .map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <Circle
                      className={`w-4 h-4 ${i === 0 ? 'text-primary fill-primary' : 'text-gray-300'}`}
                    />
                ...
                <tbody>
                  {[0, 1, 2].map(rowIdx => (
                    <tr
                      key={rowIdx}
                      className={
                        props.striped && rowIdx % 2 === 1 ? 'bg-gray-50' : ''
                      }
                    >
      ...
      case 'list':
        ...
        return (
          <ListTag
            className={`w-full h-full text-sm text-gray-700 ${props.style === 'ordered' ? 'list-decimal' : props.style === 'none' ? 'list-none' : 'list-disc'} pl-5 space-y-1`}
          >
        ...
        return (
          ...
          >
            {Array.from({ length: gridCols * 2 }).map((_, i) => (
              <div
                key={i}
                className="bg-gray-100 border-2 border-dashed border-gray-300 rounded aspect-square flex items-center justify-center"
              >
  ...
  const renderComponent = component => {
    ...
    // Note: isResizing reserved for future resize indicator styling
    const _isResizing = component.id === resizingId
    void _isResizing // Suppress unused variable warning
      ...
      >
        ...
        {isSelected && (
          <>
            ...
            {/* Resize handles */}
            <div
              className="resize-handle absolute -top-1.5 -left-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-nw-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'nw')}
            />
            <div
              className="resize-handle absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-n-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'n')}
            />
            <div
              className="resize-handle absolute -top-1.5 -right-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-ne-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'ne')}
            />
            <div
              className="resize-handle absolute top-1/2 -translate-y-1/2 -left-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-w-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'w')}
            />
            <div
              className="resize-handle absolute top-1/2 -translate-y-1/2 -right-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-e-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'e')}
            />
            <div
              className="resize-handle absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-sw-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 'sw')}
            />
            <div
              className="resize-handle absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-s-resize shadow"
              onMouseDown={e => handleMouseDownResize(e, component.id, 's')}
            />
            <div
              className="resize-handle absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-primary rounded-sm border-2 border-white cursor-se-resize shadow"
      ...
      <div className="flex-1 overflow-auto bg-[#f5f8fa] p-6">
        <div className="mx-auto" style={{ width: 'fit-content' }}>
          ...
          <div
            ...
            style={{ width: `${currentView.width + 280}px` }}
          >
            ...
            <div className="bg-[#33475b] h-12 flex items-center px-4 gap-4">
              ...
              <div className="flex-1 flex items-center gap-1 ml-4">
                {['CRM', 'Marketing', 'Sales', 'Service'].map((item, i) => (
                  <button
                    key={i}
                    className={`px-3 py-1.5 text-xs font-medium rounded ${i === 0 ? 'bg-white/10 text-white' : 'text-white/70 hover:text-white'}`}
                  >
                    {item}
                  </button>
                ))}
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-xl font-semibold text-[#33475b]">
                        {mockRecord.name}
                      </h2>
              ...
              <div className="w-[280px] bg-white border-r border-gray-200 flex-shrink-0">
                ...
                <div className="p-4 border-b border-gray-200">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                    About this {recordType}
                  </h3>
                  <div className="space-y-3">
                    {mockRecord.email && (
                      <div className="flex items-center gap-3 text-sm">
                        <Mail className="w-4 h-4 text-gray-400 flex-shrink-0" />
                        <span className="text-[#0091ae] truncate hover:underline cursor-pointer">
                          {mockRecord.email}
                        </span>
                      </div>
                    )}
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button className="text-xs text-[#0091ae] hover:underline">
                      View all
                    </button>
                  </div>
                  <div className="space-y-3">
                    {mockRecord.properties.map((prop, i) => (
                      ...
                      >
                        ...
                        <span
                          className={`font-medium ${prop.color || 'text-[#33475b]'}`}
                        >
                          {prop.value}
                        </span>
                      </div>
                    ...
                    {components.length === 0 && !isDragOver ? (
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        ...
                        <p className="text-gray-400 text-sm text-center max-w-xs">
                          Drag components from the palette on the left and drop
                          them here
                        </p>
