...
export default function AIDesignSuggestions({ isOpen, onClose }) {
  ...
  const analyzeLayout = () => {
    ...
    if (density > 0.00005) {
      suggestions.push({
        ...
        description:
          'Your card has many components in a small space. Consider using more whitespace or organizing content into sections.',
  ...
  const analyzeHierarchy = () => {
    ...
    if (textComponents.length > 0) {
      ...
      const fontSizes = textComponents.map(
        c => parseInt(c.props?.fontSize) || 14
      )
      ...
      if (uniqueSizes.size === 1) {
        suggestions.push({
          ...
          description:
            'All text components use the same font size. Consider using different sizes to create a clear hierarchy (headers, subheaders, body text).',
      ...
      if (!hasLargeText && textComponents.length > 3) {
        suggestions.push({
          ...
          description:
            "Your card lacks a prominent header. Add a larger text element to help users quickly identify the card's purpose.",
  ...
  const analyzeSpacing = () => {
    ...
    if (gaps.length > 0) {
      ...
      if (variance > 1000) {
        suggestions.push({
          ...
          description:
            'The spacing between components varies significantly. Consistent spacing creates a more professional appearance.',
          action:
            'Use consistent spacing (e.g., 16px or 24px between elements)',
          autoFix: () => {
    ...
    if (uniqueX.size > components.length * 0.7 && components.length > 3) {
      suggestions.push({
        ...
        description:
          "Components don't follow a clear alignment. Aligning to a grid creates visual order and professionalism.",
  ...
  const analyzeColors = () => {
    const suggestions = []
    const coloredComponents = components.filter(
      c => c.props?.color || c.props?.backgroundColor
    )
    ...
    if (coloredComponents.length > 5) {
      suggestions.push({
        ...
        description:
          'Using too many colors can make your design look chaotic. Stick to 2-3 primary colors plus neutrals.',
    ...
    const lowContrastText = textComponents.filter(
      c =>
        c.props?.color &&
        lightTextColors.some(lc => c.props.color.toLowerCase().includes(lc))
    )
    ...
    if (lowContrastText.length > 0) {
      suggestions.push({
        ...
        description:
          'Some text may be hard to read due to insufficient contrast with the background.',
  ...
  const analyzeContent = () => {
    ...
    const longText = textComponents.filter(
      c => (c.props?.text || '').length > 100
    )
    ...
    if (longText.length > 0) {
      suggestions.push({
        ...
        description:
          'HubSpot cards work best with concise information. Consider breaking long text into smaller chunks or using bullet points.',
        action: 'Shorten text or break into multiple components',
      })
    }

    // Check for missing content
    const emptyText = textComponents.filter(
      c => !c.props?.text && !c.propertyBinding
    )
  ...
  const analyzeBestPractices = () => {
    ...
    if (boundComponents.length === 0 && components.length > 0) {
      suggestions.push({
        ...
        description:
          "Your card doesn't use any HubSpot data. Consider binding components to CRM properties to display dynamic information.",
    ...
    if (buttons.length === 0 && components.length > 5) {
      suggestions.push({
        ...
        description:
          'Cards with clear actions (buttons) help users know what to do next. Consider adding a button for key actions.',
    ...
    if (images.length === 0 && components.length > 3) {
      suggestions.push({
        ...
        description:
          'Cards with visual elements (images, icons) are more engaging and easier to scan.',
  ...
  const handleApplySuggestion = suggestion => {
    if (suggestion.autoFix) {
      ...
      alert(
        'This suggestion requires manual adjustment. Please follow the recommended action.'
      )
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Get intelligent recommendations to improve your card
              </p>
            </div>
          </div>
          <button
              ...
              <div className="flex-1">
                <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(appliedSuggestions.length / suggestions.length) * 100}%`,
                    }}
                  ></div>
                </div>
              </div>
          ...
          {analyzing ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p className="text-sm mt-1">
                Your design looks good. No suggestions at this time.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {suggestions.map(suggestion => {
                const TypeIcon =
                  SUGGESTION_TYPES[suggestion.type]?.icon || Lightbulb
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div
                        className={`p-2 rounded ${SUGGESTION_TYPES[suggestion.type]?.color || 'bg-gray-100'}`}
                      >
        ...
        <div className="border-t border-gray-200 p-4 bg-gray-50">
          <div className="flex items-center justify-between">
            <p className="text-xs text-gray-600">
              AI suggestions are based on design best practices and HubSpot
              guidelines
            </p>
            <button
